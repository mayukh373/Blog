...
const Navbar = ({ posts}) => {
  ...
  const suggestions = (search) => {
    ...
    return (
      <div ref={items} className="flex flex-col text-sm">
        <div>{options.length ?
        ...
        })}<div onClick={() => navigate(prompt ? "?search=" + prompt : "/")} className="p-1 text-end hover:text-fuchsia-800 bg-gray-100 hover:cursor-pointer">See all</div></div> 
        : <div className="flex px-3 bg-gray-100">No results</div>}</div>
      </div>
    )
  } 

  return (
    <div className="nav-container px-4 items-center justify-between md:px-[50px] py-4 border-b-4 border-zinc-950 bg-black sticky top-0 z-10">
      ...
      {<div ref={searchBar} className="flex basis-2/3 nav-search justify-center space-x-1" style={styles.display}>
        <div onClick={() => navigate(prompt ? "?search=" + prompt : "/")} className="flex cursor-pointer me-3"><BsSearch style={{ color: "white" }} /></div>
        <div className="flex flex-col relative space-y-1 w-[200px] md:w-[300px]"> 
          <input  onChange={(e) => toSearch(e)} value={prompt} className="flex outline-none px-3 bg-gray-200" placeholder="Search" type="text" />
          <div className="flex flex-col absolute top-6 z-10 w-[200px] md:w-[300px] bg-white space-y-2">{suggestions(prompt)}</div>
        </div>
      </div>}
