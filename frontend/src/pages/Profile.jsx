...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    try {
      const res = await axios.get("http://localhost:4000/blogRoute/users/" + user._id)
  ...
  const fetchUserPosts = async () => {
    try {
      const res = await axios.get("http://localhost:4000/auth/blogRoute/posts/user/" + user._id, {headers:{'authorization': 'Bearer ' + user.token}})
      setPosts(res.data)
    }
    catch (err) {
      console.log(err)
